...
def computeDelta(wt, X, Xi):
	...
	def similarity(x, y):
		...
		for i in xrange(0, len(x)):
			sim = sim + ((x.iloc[i] - meanOfx) * (y.iloc[i] - meanOfy))
		
		sim = sim/(len(x) * np.std(x) * np.std(y))
		return sim
	
	for i in xrange(0, len(Xi)):
		tmp = math.exp(wt * similarity((X.iloc[0:(len(X) - 1)]), (Xi.iloc[i][0:len(Xi.iloc[0]) -1] )))
		num += Xi.iloc[i][len(Xi.iloc[0]) - 1] * tmp
		den = den + tmp
...
# YOUR CODE HERE
model = smf.ols('deltaP ~ deltaP90 + deltaP180 + deltaP360' , trainData).fit()
